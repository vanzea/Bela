...
4. **GrainLayer.** Short bandpass-filtered noise grains with a soft 24 ms
   attack and a squared decay. Density and bandpass centre are on Pots 4 and 5.

5. **DiffuseReverb.** A Freeverb-style topology — four parallel feedback combs
...
6. **Width crossfade.** Blends between a mono sum and the decorrelated reverb
   pair, so Pot 7 sweeps mono → fully phasey.

7. **Output stage.** A per-channel DC blocker, a fixed master trim that lands
...
| 3 | 2 | **Drive** | tanh pre-gain 1 → 14 | Saturation into harmonic haze. Past the centre this is the seismic, sub-heavy territory. |
...
- **Pure sub drone.** Grain Density fully down, Pitch low, Spread low, Drive
  low. A clean, almost sine-like foundation tone.
- **Seismic / earthquake.** Pitch low, Drive high, Diffusion long. The
  asymmetric saturation generates a dense low-order harmonic stack — this is
  the deliberately physical, room-shaking setting.
...
- **Global state** — the cluster, reverb, grain layer, control smoothers.
