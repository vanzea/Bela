...
void reverb_setDecay(Reverb* r, float d) {
    r->decay = 0.5f + 0.35f * clampf(d, 0.0f, 1.0f); /* max 0.85 */
}

float reverb_process(Reverb* r, float in) {
    /* Clamp input to prevent feeding NaN/huge values */
    if (!(in >= -10.0f && in <= 10.0f)) in = 0;
    ...
    if (!(out >= -10.0f && out <= 10.0f)) {
        out = 0;
        for (i = 0; i < 4; i++)
            memset(r->combBuf[i], 0, REV_MAX * sizeof(float));
        for (i = 0; i < 2; i++)
            memset(r->apBuf[i], 0, REV_MAX * sizeof(float));
    }
    return out;
}
